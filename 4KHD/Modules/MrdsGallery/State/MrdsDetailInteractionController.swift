...
@MainActor
@Observable
final class MrdsDetailInteractionController {
    var resetToken = UUID()
    var saveMessage = ""
    ...
    func save(item: MrdsGalleryItem, slot: MrdsImageSlot) {
    ...
    func saveVideo(item: MrdsGalleryItem, sourceURL: URL) {
        guard OnlineSourcePolicy.allows(sourceURL, source: .mrds, resource: .media) else { return }
        let panel = NSSavePanel()
        ...
        let source = SingleFileDownloadSource(
            ...
            perform: { targetURL, reportProgress in
                try await KnitVideoDownloadService.saveMP4(
                    ...
                    referer: MrdsRequestFactory.htmlOrigin
                ) { progress in
                    reportProgress(
