...
enum QuanjiRequestFactory {
    ...
    nonisolated static let htmlOrigin = "https://91quanji.com/"

    static func makeHTMLRequest(
        url: URL,
        cachePolicy: URLRequest.CachePolicy = .reloadIgnoringLocalCacheData
    ) throws -> URLRequest {
        try OnlineSourcePolicy.validate(url, source: .quanji, resource: .html)
    ...
    nonisolated static func configureImageRequest(_ request: inout URLRequest) {
        guard let url = request.url,
              OnlineSourcePolicy.allows(url, source: .quanji, resource: .media)
        else {
            request.url = nil
            return
        }
...
enum QuanjiHTTPClientError: LocalizedError {
    case badStatus(Int)

    var errorDescription: String? {
        switch self {
        case let .badStatus(status): "木瓜视频服务器返回状态码 \(status)"
        }
    }
}

enum QuanjiHTTPClient {
    static func data(for request: URLRequest) async throws -> (Data, HTTPURLResponse) {
        let (data, response) = try await OnlineSourceSession.quanjiHTML.data(for: request)
        guard let response = response as? HTTPURLResponse else {
            throw URLError(.badServerResponse)
        }
        guard (200 ..< 300).contains(response.statusCode) else {
            throw QuanjiHTTPClientError.badStatus(response.statusCode)
        }
