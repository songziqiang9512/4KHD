...
@MainActor
@Observable
final class KnitGalleryStore {
    typealias ListResolver = (KnitListContext, Int) async throws -> KnitListPage
    typealias DetailResolver = (URL) async throws -> KnitResolvedDetailPage

    private(set) var filter: KnitBrowseFilter = .all
    var searchText = ""
    private(set) var activeSearchQuery: String?
    private(set) var items: [KnitGalleryItem] = []
    private(set) var selectedItemID: KnitGalleryItem.ID?
    ...
    private(set) var imageSlots: [KnitImageSlot] = []
    private(set) var selectedImageIndex = 0
    private(set) var detailErrorMessage: String?
    private(set) var detailMetadata: KnitDetailMetadata?
    ...
    @ObservationIgnored private var selectedItemSnapshot: KnitGalleryItem?
    ...
    init(
        ...
        listResolver: @escaping ListResolver = KnitListResolver.resolve(context:page:),
        detailResolver: @escaping DetailResolver = KnitDetailResolver.resolve(pageURL:)
    ) {
    ...
    // 保留旧命名，供现有调用点平滑迁移。
    var canStepImageBackward: Bool { canStepDetailBackward }
    var canStepImageForward: Bool { canStepDetailForward }
    ...
    var favoriteItemIDs: Set<KnitGalleryItem.ID> {
    ...
    func setFilter(_ filter: KnitBrowseFilter) {
    ...
    func refreshFromNetwork() {
        let context = activeSearchQuery.map(KnitListContext.search) ?? .filter(filter)
    ...
    func select(_ item: KnitGalleryItem, force: Bool = false) {
    ...
    func openRecommendation(_ recommendation: OnlineGalleryRecommendation) {
        let item = KnitGalleryItem(
            ...
            coverAspectRatio: recommendation.coverAspectRatio ?? 0.75,
            reportedPhotoCount: recommendation.imageCount ?? 0,
            reportedGIFCount: 0,
    ...
    func isFavorite(_ item: KnitGalleryItem) -> Bool {
        favorites.contains(detailURL: item.detailURL)
    }

    func toggleFavorite(for item: KnitGalleryItem) async throws {
        try await favorites.toggle(KnitFavoritesBridge.record(from: item, metadata: detailMetadata))
    }

    private func refresh(context: KnitListContext) {
        listTask?.cancel()
        listTask = nil
    ...
    private func applyListPage(_ page: KnitListPage, replacing: Bool) {
        if replacing {
    ...
    private var currentListContext: KnitListContext {
        activeSearchQuery.map(KnitListContext.search) ?? .filter(filter)
    }

    private func prepareDetail(for item: KnitGalleryItem) {
        detailTasks.values.forEach { $0.cancel() }
        detailTasks.removeAll()
    ...
    private static func pageNumber(from url: URL) -> Int? {
        ...
        guard let pageIndex = components.lastIndex(of: "page"),
              components.indices.contains(pageIndex + 1) else { return nil }
        return Int(components[pageIndex + 1])
    }

    private static func itemID(from detailURL: URL) -> String {
        let components = detailURL.pathComponents.filter { $0 != "/" }
        if let articleIndex = components.lastIndex(of: "article"),
           components.indices.contains(articleIndex + 1) {
            return components[articleIndex + 1]
        }
