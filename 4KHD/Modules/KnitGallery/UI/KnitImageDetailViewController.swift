...
@MainActor
final class KnitImageDetailViewController: NSViewController, WorkspaceFocusable {
    ...
    private let interaction: KnitDetailInteractionController
    private let filmstripVisibility: FilmstripVisibilityController
    private let onPlayVideo: (KnitGalleryItem, URL) -> Void
    private let imageView = KnitZoomableImageView()
    private let recommendationsView = DetailRecommendationsView()
    private let filmstripView = KnitFilmstripView()
    ...
    private var currentSlotID: KnitImageSlot.ID?
    private var currentImageURL: URL?
    private var resetTokenSeen = UUID()
    private var isObserving = false
    private var isViewActive = false
    private let reloadQueue = WorkspaceCoalescingQueue(name: "Knit Detail Reload", interval: 0.04, maxInterval: 0.1)

    init(
        store: KnitGalleryStore,
        immersive: ImmersiveController,
        detailPane: WorkspaceDetailPaneController,
        interaction: KnitDetailInteractionController,
        filmstripVisibility: FilmstripVisibilityController,
        onPlayVideo: @escaping (KnitGalleryItem, URL) -> Void
    ) {
        self.store = store
    ...
    private func reloadDetail() {
        ...
        recommendationsView.update(
            ...
            requestConfigurator: KnitRequestFactory.configureImageRequest
        )
    ...
    private var validatedVideoURL: URL? {
        guard let url = store.videoURL,
              ...
              OnlineSourcePolicy.allows(url, source: .knit, resource: .media) else { return nil }
        return url
    }

    @objc private func previousImage() { store.stepImage(-1) }
    @objc private func nextImage() { store.stepImage(1) }
